...
subroutine nnmodel_mpi(fvec,n,m,dlocal,d,c,cgrad)
  ...
  !Set dinv to use in later calculations
  dinv = 1.d0/dble(d)

  !Compute inner layer activation vector, a_inner
  z_inner = matmul(w_inner,nm_x)

  !dlocal is calculated in the main file
  !d iterations are distributed across proccessors during parallellization
  !Each process iterates over dlocal
  do i1=1,dlocal
    z_inner(:,i1) = z_inner(:,i1) + b_inner
  end do
  !Computes inner layer activation vector, a_inner
...
subroutine fgd_mpi(comm,fvec_guess,n,m,dlocal,d,alpha,fvec)
  ...
  real(kind=8), dimension(size(fvec_guess)) :: cgrad,sumc_grad
  integer :: myid,ierr
  ...
  do i1=1,i1max
    ...
    !Reduce fvec, sum the values of fvec generated by each process during parallellization
    !sumc_grad is total of the sum of fvec from each process
    call MPI_REDUCE(cgrad,sumc_grad,n_fvec,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)

    !update fvec
    if (myid==0) then
    fvec = fvec - alpha*sumc_grad
    end if

    !Broadcast a copy of fvec to make it available on each process
    call MPI_BCAST(fvec, n_fvec, MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
